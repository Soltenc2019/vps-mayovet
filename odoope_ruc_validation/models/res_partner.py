...
class ResPartner(models.Model):
    ...
    @api.model
    def ruc_soltecn_connection(self, ruc):
        session = requests.Session()
        headers = requests.utils.default_headers()
        headers['User-Agent'] = 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/56.0.2924.87 Safari/537.36' 
        url_ruc = "https://dniruc.apisperu.com/api/v1/ruc/{ruc}?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJlbWFpbCI6ImRlc2Fycm9sbG8yQHNvbHRlY24uY29tIn0.odI7UTknsnzF8pfbgvafDi-XNqdC3x5UwFBECuTrNX4"
        data = {}
        try:
            result= session.get(url=url_ruc.format(ruc=ruc),verify = False,headers=headers).json()
            data['ruc'] = result["ruc"]
            data['business_name'] = result["razonSocial"]
            data['type_of_taxpayer'] = False
            data['estado'] = result["estado"]
            data['contributing_condition'] = result["condicion"]
            data['commercial_name'] = result["nombreComercial"]

            district = False
            if result["distrito"]:
                district = (result["distrito"]).title()
            province = False
            if result["provincia"]:
                province = (result["provincia"]).title()
            prov_ids = self.env['res.city'].search([('name', '=', province),('state_id','!=',False)])
            dist_id = self.env['l10n_pe.res.city.district'].search([('name', '=', district),('city_id', 'in', [x.id for x in prov_ids])], limit=1)
            dist_short_id = self.env['l10n_pe.res.city.district'].search([('name', '=', district)], limit=1)
            if dist_id:
    ...
    def l10n_pe_ruc_connection(self, ruc):
        ...
        if self.env.user.company_id.l10n_pe_api_ruc_connection == 'soltecn_api':  
            data = self.ruc_soltecn_connection(ruc)
        if self.env.user.company_id.l10n_pe_api_ruc_connection == 'soltecn_api_2':
            data = self._get_status_api("/v1/ruc", {"numero": ruc}, "RUC")
        return data       
    ...
    def _get_status_api(self, path: str, params: dict, tipo: str):

        url = f"{self.BASE_URL}{path}"
        
        headers = {
            "Authorization": self.APIS_TOKEN, 
        ...
        if response.status_code == 200:
            ...
            if tipo == "RUC":
                data['ruc'] = resultado_json['numeroDocumento']
                data['business_name'] = resultado_json['nombre']
                data['commercial_name'] = resultado_json['nombre']               
                data['residence']  = resultado_json['direccion']
                data['contributing_condition'] = resultado_json['condicion']
                provincia = resultado_json['provincia'].title()
                distrito = resultado_json['distrito'].title()
                prov_ids = self.env['res.city'].search([('name', '=', provincia),('state_id','!=',False)])
                dist_id = self.env['l10n_pe.res.city.district'].search([('name', '=',distrito ),('city_id', 'in', [x.id for x in prov_ids])], limit=1)
                dist_short_id = self.env['l10n_pe.res.city.district'].search([('name', '=', resultado_json['distrito'])], limit=1)
                if dist_id:
                    l10n_pe_district = dist_id
                else:
                    l10n_pe_district = dist_short_id
    ...
    @api.model
    def dni_soltecn_connection(self, dni):
        session = requests.Session()
        headers = requests.utils.default_headers()
        headers['User-Agent'] = 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/56.0.2924.87 Safari/537.36'
        url_dni = 'https://dniruc.apisperu.com/api/v1/dni/{dni}?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJlbWFpbCI6ImRlc2Fycm9sbG8yQHNvbHRlY24uY29tIn0.odI7UTknsnzF8pfbgvafDi-XNqdC3x5UwFBECuTrNX4'
        data = {}
        try:
            result= session.get(url=url_dni.format(dni=dni),verify = False,headers=headers).json()
            data['nombre'] = (result['nombres'] + " " +result['apellidoPaterno'] + " " + result['apellidoMaterno'])
        except Exception:
            self.alert_warning_vat = True
            data = False 
        return data
    ...
    def l10n_pe_dni_connection(self, dni):
        ...
        elif company.l10n_pe_api_dni_connection == 'soltecn_api':
            data = self.dni_soltecn_connection(dni)
        elif company.l10n_pe_api_dni_connection == 'soltecn_api_2':
            data = self._get_status_api("/v1/dni", {"numero": dni}, "DNI")
        else:
            data = False
        return data
